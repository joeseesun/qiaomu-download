...
5. **yt-dlp official README, installation wiki and FAQ** — authoritative basis for extractor breadth, browser Cookie support, package-manager update routes and the fact that site support must be tested dynamically.
...
- WeChat UI automation; video-number links route to qiaomu-wx-video.
...
- A platform router that keeps WeChat Channels outside the generic downloader.
...
- A single JSON contract shared by YouTube, Bilibili, X and generic extractors.

## Claim status

- **Design advantage:** one command surface, public-first privacy policy, verified outputs, conservative single-link scope and explicit WeChat isolation.
