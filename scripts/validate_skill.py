...
def main() -> None:
    ...
    for term in ("Trust boundary", "Rollback boundary", "qiaomu-wx-video", "不得使用 Computer Use", "小红书", "安全验证", "最多尝试一次 Cookie 回退"):
    ...
    if manifest.get("name") != "qiaomu-download" or manifest.get("version") != "1.1.1":
