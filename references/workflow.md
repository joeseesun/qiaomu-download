...
2. `route`：视频号链接交给 qiaomu-wx-video。
3. `dependency`：安装缺失依赖后重试。
4. `metadata`：更新 yt-dlp；需要登录时保持浏览器已登录并允许 Cookie 回退。
5. `download`：检查网络、磁盘空间和站点可用性；同一媒体的并发任务会被锁拒绝。
6. `verify`：文件未通过 ffprobe，不宣告成功。
